...
mod imp {
    ...
    pub(super) fn attach_device(lbx_name: &str, device_path: &str) -> Result<PathBuf> {
...
mod imp {
    ...
    pub(super) fn attach_device(_: &str, _: &str) -> Result<PathBuf> {
        bail!("Device attachment is only supported on Linux hosts")
    }
}

pub fn attach_device(lbx_name: &str, device_path: &str) -> Result<PathBuf> {
    imp::attach_device(lbx_name, device_path)
}
